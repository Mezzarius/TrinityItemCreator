...
namespace TrinityItemCreator.MyClass
{
    class QueryHandler
    {
        ...
        public static string GetExportQuery()
        {
            ...
            myData.GetItemTemplateColumnNames();
            string QColumns = string.Empty;
            foreach (var item in MyData.item_template_columns) { QColumns += $"`{item}`,"; }

            string VQuery = "-- Item created with TrinityItemCreator\n" +
                sqlPrefix +
                $" INTO `item_template` ({QColumns.Remove(QColumns.Length - 1)}) VALUES "
